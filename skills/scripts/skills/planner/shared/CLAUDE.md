...
| `resources.py`          | Convention loading, script path resolution, state dir validation AND placement (`resolve_state_dir`, `resolve_project_root`, `.gitignore` gating incl. the tracked-files refusal and the post-creation ignore re-check, run-dir retention) | Adding a conventions file, changing resource paths, changing where state dirs live, how the project root is found, or how old run dirs are pruned |
