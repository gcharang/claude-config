...
| `conftest.py`                 | Pytest config and shared helpers: `temp_root`, `_git_repo`, `requires_unprivileged`, the session leak guard, the all-skipped backstop | Modifying test setup, adding a fixture, changing what the leak guard watches |
...
| `leak_guard.py`               | Session guard: snapshots the projects the suite can reach, fails on any write into them; also the all-skipped backstop | Adding a test that mints a state dir or archives a plan; changing what counts as all-skipped |
