...
The working directory is only trustworthy where the caller has not been `cd`'d away from the project first, which is **step 1 alone**. Its `SKILL.md` invocation therefore uses `uv run --project <SKILLS_DIR> python -m ...` rather than `<invoke working-dir=...>`: `working-dir` resolves the install layout by `cd`-ing into it, which discards the one signal step 1 has. Claude Code populates `$CLAUDE_PROJECT_DIR` for hook subprocesses but not for the Bash-tool subprocesses these scripts run in, so in practice the working directory is what resolves.
